...
def run_drg(solution_object, conditions_file, error_limit, target_species,
            retained_species, model_file, final_error, thresh=None):
    """
    ...
    helper.progress(0.0,"ReC",1)
    conditions_array = readin_conditions(conditions_file)

    helper.progress(0.0,"SeS",1)
...
def drg_loop_control(solution_object, target_species, retained_species, model_file, stored_error, threshold, done, rate_edge_data,
    ...
    maxerr = np.max(error)
    print("\r                                                                     ",end='')
    print("\rThreshold\tSpecies in Mech\t\tError")
    print("%.3f\t\t%d\t\t\t%.3f"%(threshold,len(new_solution_objects[1].species()),maxerr))
    stored_error[0] = round(maxerr, 3)
...
def get_rates_drg(sim_array, solution_object):
    ...
    for i,ic in enumerate(sim_array):
        ...
        for tstep in ic.sample_points:  # Iterate through all timesteps
            ...
            denom = {}
            numerator = {}
            # for spc in solution_object.species():
            for i, reac in enumerate(solution_object.reactions()):
                reac_prod_rate = float(rate_progress[i])
